...
public class Scenario extends KinectWorld
{
         ...
         if(krrish.getLives() > 0)
        {
            ...
            else if(level == 2 /*&& contCambioNivel == 0*/)
            {
                ...
                // {
                   // levelCnt++;    
                // }
                if(numEnemies == 0 && krrish.getLives() >= 1 /*&& levelCnt == 3*/)
                {
                    levelCnt=0;
                    level++;
            ...
            else if(level == 3)
            {
                ...
                if(levelCnt == 0)
                {
                   ...
                   // krrish.setEnergia(100);
                    krrish.setNumShots(krrish.getNumShots() + 200); 
                    krrish.setHealth(50);
                ...
                cntHealthKaal.setPrefix("KAAL: " + gb.getHealth() + "/80");
            }
        }
        else if(krrish.getLives() <= 0 && lostPlayer == false) 
        {
    ...
    private void prepare()
    {
        ...
        cntLife.setValue(krrish.getLives());
        cntHealth.setValue(krrish.getHealth());
        cntShots.setValue(krrish.getNumShots());
        cntEnemies.setPrefix("Enemies = " + numEnemies + "/");
        cntEnemies.setValue(54);
        cntPoints.setValue(krrish.getPoints());
    ...
    public void prepareLevel3()
    {
        ...
        cntHealthKaal = new Counter();
        cntHealthKaal.setPrefix("KAAL: " + gb.getHealth() + "/80");
        addObject(cntHealthKaal, 925, 485);
    }
    
    
    public Krrish getKrrish()
    {
        return krrish;
    }
